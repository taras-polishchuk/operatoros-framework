 ...
 * (the // __EMBEDDED_RUNTIME__ block is empty). At ncc-build time,
 * `scripts/embed-assets.js` runs first and replaces the markers' contents
 * with literal preset YAML + schema JSON so the single-file binary becomes
...
export function installEmbeddedAssets(): void {
  ...
  "personal": "version: \"0.2\"\nname: personal\ndescription: |\n  Default OperatorOS workspace. Installs two bundled example modules\n  (journal, timer) so `operatoros apply` produces something visible\n  immediately. Override with `operatoros add <path-or-url>` for your own.\nmodules:\n  - name: journal\n    source: ../../examples/journal\n  - name: timer\n    source: ../../examples/timer\nsettings:\n  vault_path: vault/\n  state_path: state/\n  export:\n    deny:\n      - \"vault/**\"\n      - \"**/.env\"\n      - \"**/.env.*\"\n      - \"**/*.sqlite\"\n      - \"**/*.sqlite-wal\"\n      - \"**/*.sqlite-shm\"\n      - \"**/id_rsa\"\n      - \"**/id_rsa.pub\"\n      - \"**/secrets.yaml\""
};
